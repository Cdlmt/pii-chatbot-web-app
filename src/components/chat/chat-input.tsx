  ...
  function ChatInput({ value, onChange, onSubmit, isLoading }, ref) {
    return (
      <div className="absolute bottom-0 left-0 right-0 p-6">
        <div className="absolute inset-x-0 bottom-0 h-40 bg-linear-to-t from-background via-background to-transparent pointer-events-none" />

        <form onSubmit={onSubmit} className="relative max-w-3xl mx-auto">
          <div className="glass-panel-strong rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl focus-within:shadow-xl focus-within:border-(--border-hover)">
            <div className="flex items-center gap-3 p-2">
              <input
                ...
                className="flex-1 bg-transparent px-4 py-3 text-foreground placeholder:text-(--foreground-subtle) focus:outline-none text-base"
                disabled={isLoading}
              />
              <button
                type="submit"
